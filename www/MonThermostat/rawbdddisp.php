...
<title>RAW Data from DB</title>
</head><body>
<?php

echo '>>Zone NB (ThZoneNb): '.$ThZoneNb.'<BR>';
echo '>>Day NB (DayNb): '.$DayNb.'<BR>';
echo '>>SetPointDayId: '.$SetPointDayId.'<BR>';
echo '>>Delete SPD (DeleteSpd): '.$DeleteSpd.' <BR>';
echo '--SwiMode ['.$swimode.']--<BR>';
echo '>>IdSetPointDay: '.$idSetPointDay[0].' ['.$SPDName[0].']<BR>';
echo '>>ZoneSPDTabName: '.$SPDName[0].'<BR> :[ '.$ZoneSPDTabTime[0][23].' ]<BR>';
echo '>>Timestamp Now: '.mktime().' Timestamp Deamon: '.$HBTimeStamp.' ['.MTH_deamonOK(mktime(),$HBTimeStamp,60).']<BR>';
?>
</body>
</html>

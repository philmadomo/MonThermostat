...
function display_LoadButtons($bgcolorval,$zonenb,$daynb,$link,$idSPDarray,$SPDnamearray,$loadbttext,$Deletbttest)
{
        echo ("<table bgcolor=\"$bgcolorval\" border=0 cellpadding=0 cellspacing=0>\n<tbody>\n");
        print("<tr>\n");
        display_cellval(0,0,1,48);
	...
	foreach ($idSPDarray as $key => $idSPD) {
		echo '<option value="'.$idSPDarray[$key].'">'.$SPDnamearray[$key].'</option>';
	}
	echo '
	</select>&nbsp;
...
echo ("</script>\n");
# -2x+49 = Y x en C et y en IndexTab
# (49-x)/2=y x en IndexTab et y en C

#TEST PURPOSE !!! ONLOAD !!!
echo ("<title>Test TAB</title>\n</head>\n<body bgcolor=#F1F1F1 onload=\"javascript:LoadTabSetPoint(SetPointTab,sizetabx);\">\n");

#Display HTML Tab + CSS
echo ("<table bgcolor=\"$bgcolorval\" border=0 cellpadding=0 cellspacing=0>\n<tbody>\n");
display_tabtiles($tiledline1,$sizetabx+3,$sizetaby+3,return_indexval_ifdayok(date("G"),date("i"),date("N"),$dayselected));
//display_tabtiles($tiledline1,$sizetabx+3,$sizetaby+3,return_indexval_ifdayok(0,10,date("N"),2));
//display_tabtiles($tabarray,$tabsizex,$tabsizey,$indexval)
$hournow=date("G");
$minutenow=date("i");
$daynumber=date("N");
...
echo ("</tbody>\n</table>\n");
display_daylinebuttons($bgcolorval,$configval,date("N"),"loadday.php?day");
...
print("<tr>\n");
print("<td>\n");	
display_LoadButtons($bgcolorval,$zoneselected,$dayselected,"rawbdddisp.php",array(3,4,6,13),array("Travail","Dimanche","Samedi","Aeffacer"),"Load","Delete");
//display_LoadButtons($bgcolorval,$zonenb,$daynb,$link,$idSPDarray,$SPDnamearray,$loadbttext)
print("</td><td>&nbsp;</td><td>\n");
display_SaveButtons($bgcolorval,$zoneselected,$dayselected,"6","rawbdddisp.php","Save");
//display_SaveButtons($bgcolorval,$zonenb,$daynb,$spdid,$link,$savebttext)
print("</td><td>&nbsp;</td><td>\n");
display_ResetButtons($bgcolorval,$zoneselected,$dayselected,"SetPointTab","Reset");
//display_ResetButtons($bgcolorval,$zonenb,$daynb,$jsvar,$resetbttext)
print("</td>\n");
print("</tr><tr><td>&nbsp;</td></tr><tr><td>\n");
display_CreateButtons($bgcolorval,$zoneselected,$dayselected,"rawbdddisp.php","New-SpdName","Create");
//display_CreateButtons($bgcolorval,$zonenb,$daynb,$link,$SPDnamenew,$creabttext)
print("</td><td>&nbsp;</td><td>");
$tps_end = get_microtime();
$tps = $tps_end - $tps_start;
display_MessageWindows($bgcolorval,0,"56",round($tps,3));
//display_MessageWindows($bgcolorval,$statMTH,$secondsince,$loadtime)
print("</td><td></td><td>\n");
print("<tr>\n");
echo ("</tbody>\n</table>\n");

printf("%d:%2d day=%d\n",$hournow,$minutenow,$daynumber);
printf("index=%d <BR>\n",return_indexval_ifdayok(date("G"),date("i"),date("N"),$dayselected));

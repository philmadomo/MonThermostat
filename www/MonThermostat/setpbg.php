...
<script language="JavaScript">
    ...
    document.getElementById(el).style.background = "url(img/casev.png)";
    }
    
    function HLOneNotSelectc(el){
    document.getElementById(el).style.background = bgnotselect;
    }
    
    function HLOneNotSelectt(el){
    document.getElementById(el).style.background = "url(img/caset.png)";
    }
	...
	if ( j % 2 == 0) {
		if ( Pox % 2 ) {
  		HLOneNotSelect(prefixeid.concat(Pox,postfixid).concat(j));
  		}
		else
		{
		HLOneNotSelectt(prefixeid.concat(Pox,postfixid).concat(j));
		}
	}
	else
	{
		HLOneNotSelectc(prefixeid.concat(Pox,postfixid).concat(j));
	}
	j--;
  	}
		while (i<=maxtb)
